...
var profileCheck = function profileCheck(request, response, profileAllowed) {
...
* profile is in valid
*/
var isUser = function isUser(request, response, next) {
  var reply = profileCheck(request, response, 'User');
  if (reply) {
    return reply;
  }
  next();
...
* profile is in valid
*/
var isAdmin = function isAdmin(request, response, next) {
  var reply = profileCheck(request, response, 'Admin');
  if (reply) {
    return reply;
  }
  next();
